...
*/
    ...
    public void get01() {
        ...
        response.
                ...
                statusCode(200).
                body("data.id",
                        hasSize(24), "data.employee_name", hasItems("Tiger Nixon", "Garrett Winters"));
        ...
        for (Integer w: salaryList){
            sum=sum+w;                                                                                  !!!GOOD
        }  instead of for loop we shoul use Lambda,lets see below==>>     */

        int sum=salaryList.stream().reduce(0,(x,y)->(x+y));//first way to calculate the sum       !!!BETTER
        int sum2=salaryList.stream().reduce(0,Math::addExact);//this is the second way of using lambda,and better than first way     !!!BEST


        assertEquals(6644770,sum);

        //before push github delete all (sout) and comments.we added all comments and deleted it
    }
